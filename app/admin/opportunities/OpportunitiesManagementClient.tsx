...
const DEFAULT_OPPORTUNITIES_SEED: OpportunityItem[] = [];

const OPPORTUNITY_TYPES = [
  { id: 'internship', label: 'Industrial Internship' },
  { id: 'skill_camp', label: 'Skill Camp & Bootcamp' },
  { id: 'grant', label: 'Grant & Fellowship' },
  { id: 'job', label: 'Graduate Employment' },
];

export default function OpportunitiesManagementClient({
  initialOpportunities = []
}: {
  initialOpportunities?: OpportunityItem[];
}) {
  const [opportunitiesList, setOpportunitiesList] = useState<OpportunityItem[]>(initialOpportunities);
  ...
  // Selection handlers
  const isAllSelected = filteredOpportunities.length > 0 && filteredOpportunities.every((it) => selectedIds.includes(it.id));

  const handleToggleSelectAll = () => {
    if (isAllSelected) {
      setSelectedIds([]);
    } else {
      setSelectedIds(filteredOpportunities.map((it) => it.id));
    }
  };
  ...
  const handleOpenCreateModal = () => {
    ...
    setFormData({
      ...
      link: 'https://gnuts.org.gh/opportunities',
      status: 'active',
    });
    setFeedbackMsg('');
    setIsModalOpen(true);
  ...
  const handleOpenEditModal = (item: OpportunityItem) => {
    setEditingItem(item);
    setFormData({
      title: item.title,
  ...
  const handleSaveOpportunity = async (e: React.FormEvent) => {
    ...
    try {
      if (editingItem) {
        // Edit Mode
        const res = await fetch(`/api/admin/opportunities/${editingItem.id}`, {
          method: 'PUT',
        ...
        if (res.ok) {
          ...
          setFeedbackMsg('Failed to update opportunity');
        }
      } else {
        // Create Mode
        ...
        if (res.ok && data.opportunity) {
          setOpportunitiesList([data.opportunity, ...opportunitiesList]);
          setIsModalOpen(false);
        } else {
          setFeedbackMsg('Failed to post opportunity');
        }
      }
    } catch {
  ...
  const handleToggleStatus = async (item: OpportunityItem) => {
    const newStatus = (item.status || 'active').toLowerCase() === 'active' ? 'closed' : 'active';
    try {
      await fetch(`/api/admin/opportunities/${item.id}`, {
        method: 'PUT',
        headers: { 'Content-Type': 'application/json' },
  ...
  const handleDeleteOpportunity = async (id: number) => {
    try {
      ...
      if (res.ok) {
        setOpportunitiesList((prev) => prev.filter((item) => item.id !== id));
  ...
  const handleBulkDeleteOpportunities = async () => {
    ...
    try {
      ...
      if (res.ok) {
        setOpportunitiesList((prev) => prev.filter((item) => !selectedIds.includes(item.id)));
        setSelectedIds([]);
        setIsBulkDeleteModalOpen(false);
      } else {
        alert('Failed to bulk delete selected opportunities');
      }
    } catch {
    ...
    <div className="space-y-6 font-['Montserrat',sans-serif] pb-20">
      ...
      <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4">
        <div>
          ...
          <p className="text-xs sm:text-sm text-gray-600 font-medium mt-0.5">
            Post TVET industrial attachments, graduate trainee positions, and skill acquisition bootcamps ({opportunitiesList.length} total)
          </p>
        </div>
      ...
      <div className="bg-white rounded-3xl p-4 border border-gray-200 shadow-sm flex flex-col md:flex-row items-center justify-between gap-4">
        <div className="flex items-center gap-3 w-full md:w-auto">
          {filteredOpportunities.length > 0 && (
            <button
              ...
              title={isAllSelected ? 'Deselect all opportunities' : 'Select all opportunities for bulk actions'}
            >
              {isAllSelected ? (
        ...
        <div className="flex flex-wrap items-center gap-2 w-full md:w-auto">
          ...
          <select
            ...
            className="px-3 py-1.5 bg-gray-100 rounded-xl text-xs font-bold text-gray-700 outline-none border border-gray-200 cursor-pointer"
          >
            <option value="ALL">All Categories</option>
            <option value="internship">Internships</option>
            <option value="skill_camp">Skill Camps & Training</option>
            <option value="grant">Grants & Fellowships</option>
            <option value="job">Graduate Jobs</option>
          </select>
        </div>
      </div>
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
        {filteredOpportunities.map((item) => {
          ...
          const typeObj = OPPORTUNITY_TYPES.find((t) => t.id === item.type?.toLowerCase()) || { label: item.type || 'Opportunity' };
          ...
          return (
            ...
            >
              ...
              <div className="space-y-2.5">
                <div className="flex items-start justify-between gap-2">
                  <div className="flex items-center gap-2">
                    <button
                      ...
                      title={isSelected ? 'Deselect opportunity' : 'Select opportunity for deletion'}
                    >
                      {isSelected ? (
                        <CheckSquare className="w-4 h-4 text-[#014900]" />
                      ) : (
                    ...
                    <span className="px-3 py-1 bg-emerald-50 text-[#014900] text-[10px] font-black uppercase tracking-wider rounded-full border border-emerald-200 flex items-center gap-1">
                ...
                <p className="text-xs text-gray-600 font-medium leading-relaxed line-clamp-3 pt-1">
                  {item.description}
                </p>
              </div>
              ...
              <div className="pt-3 border-t border-gray-100 flex items-center justify-between gap-2 text-xs">
                <div className="flex items-center gap-1.5 text-gray-500 font-semibold text-[11px]">
                  ...
                  <span>
                    Deadline: <strong className="text-gray-900">{item.deadline ? String(item.deadline).substring(0, 10) : 'Open Rolling'}</strong>
                  </span>
                </div>
        ...
        <div className="fixed bottom-6 inset-x-4 sm:left-1/2 sm:-translate-x-1/2 sm:w-full sm:max-w-xl z-40 bg-gray-950/95 text-white backdrop-blur-md px-6 py-4 rounded-3xl shadow-2xl border border-white/20 flex items-center justify-between gap-4 animate-fadeIn">
          <div className="flex items-center gap-3">
            ...
            <span className="text-xs font-black uppercase tracking-wider text-gray-200">
              {selectedIds.length === 1 ? '1 opportunity selected' : `${selectedIds.length} opportunities selected`}
            </span>
          </div>
              ...
              <div className="flex items-center gap-2.5">
                ...
                <div>
                  <h3 className="text-base sm:text-lg font-black text-[#014900] uppercase tracking-tight">
                    {editingItem ? 'Edit Opportunity' : 'Post New Opportunity'}
                  </h3>
                  <p className="text-xs text-gray-500 font-medium">TVET Internships, Trainee Roles & Bootcamps</p>
                </div>
              </div>
              <button
                type="button"
            ...
            <form onSubmit={handleSaveOpportunity} className="space-y-4">
              ...
              <div className="space-y-1">
                ...
                <input
                  ...
                  placeholder="e.g. Electrical Engineering Industrial Internship 2026"
              ...
              <div className="grid grid-cols-1 sm:grid-cols-2 gap-3">
                ...
                <div className="space-y-1">
                  ...
                  <input
                    ...
                    placeholder="e.g. Takoradi & Remote, Ghana"
              ...
              <div className="space-y-1">
                <label className="text-xs font-black uppercase tracking-wider text-gray-700">
                  Role Description & Candidate Requirements *
                </label>
                <textarea
                  required
                  rows={5}
                  ...
                  placeholder="Provide details about the training program, internship responsibilities, eligibility qualifications..."
              ...
              <div className="flex items-center justify-end gap-3 pt-4 border-t border-gray-100">
                ...
                >
                  {isSaving ? 'Saving...' : (editingItem ? 'Update Opportunity' : 'Post Opportunity')}
                </button>
              </div>
      ...
      {isDeletingId && (
        <div className="fixed inset-0 z-50 flex items-center justify-center p-4 bg-black/60 backdrop-blur-xs font-['Montserrat',sans-serif]">
          <div className="bg-white rounded-3xl max-w-sm w-full p-6 shadow-2xl border border-gray-200 text-center space-y-4 animate-scaleUp">
            ...
            <h4 className="text-base font-black text-gray-900">Remove this Opportunity?</h4>
            <p className="text-xs text-gray-600 font-medium">
              This opportunity will be permanently removed from the public website.
            </p>
            <div className="flex items-center justify-center gap-3 pt-2">
              ...
              <button
                type="button"
                onClick={() => handleDeleteOpportunity(isDeletingId)}
        ...
        <div className="fixed inset-0 z-50 flex items-center justify-center p-4 bg-black/60 backdrop-blur-xs font-['Montserrat',sans-serif]">
          <div className="bg-white rounded-3xl max-w-md w-full p-6 sm:p-8 shadow-2xl border border-rose-100 space-y-4 animate-scaleUp">
            <div className="flex items-center gap-3 text-rose-600">
              ...
              <div>
                ...
                <span className="text-xs font-bold text-rose-600">
                  {selectedIds.length} opportunity record(s) selected
                </span>
              </div>
            </div>

            <p className="text-xs text-gray-600 font-medium leading-relaxed">
              Are you sure you want to permanently delete <strong className="text-gray-900">{selectedIds.length} selected opportunities</strong>? This action cannot be undone.
            </p>
            ...
            <div className="flex items-center justify-end gap-3 pt-2">
              ...
              <button
                type="button"
                onClick={handleBulkDeleteOpportunities}
                disabled={isSaving}
              ...
              >
                {isSaving ? 'Removing...' : `Delete ${selectedIds.length} Opportunities`}
              </button>
            </div>
          </div>
        </div>
